...
│   │   └── shared/{Toast, ErrorBanner, PaneDivider}.tsx
│   ├── lib/{api.ts, history.ts, preferences.ts, types.ts, useAnalysisState.ts, format.ts}
│   │                         # format.ts is additive: label/number/relative-time helpers.
│   │                         # preferences.ts is a second localStorage external store, for
│   │                         # layout prefs only — it is not part of AnalysisController
