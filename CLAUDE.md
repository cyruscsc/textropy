...
The **backend is implemented** against `specifications/specs_mvp.md` (all 20 spec features across both modes and three tiers, plus twenty-four Tier 1 features added afterwards — the lemma and density pairs, four clause counts (`specs_features.md` §3), eleven sentence features (§4) and five punctuation features (§5) — `specs_mvp.md` has not been amended). **`specifications/specs_features.md` is the source of truth for linguistic features**: it defines every implemented and planned feature, and §11 records five accepted decisions that must not be silently reversed. Section 6 (complexity) is specified but not yet implemented. The **frontend is scaffolded and working end-to-end** — Next.js 16 (App Router) + React 19 + Tailwind v4, all of §11's components and both layouts (three-pane and the sub-1024px tabbed fallback). A root `compose.yml` deploys both services. Still missing: any frontend tests.
