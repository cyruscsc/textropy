...
**Layout (§9).** Three panes: History (fixed ~280px) · Analysis configuration (fluid ~40%) · Results (remaining). Panes are divided by a hairline `1px solid var(--border)`, never drop shadows. Below ~1024px the layout collapses to a single column with **History / Analyze / Results** as top-level tabs, rendered by `components/shared/PaneTabBar.tsx` as a rounded-pill icon menu floating over the *bottom* of the viewport (thumb reach), with Analyze as the centre item and the default pane. Every pane is header-plus-scroll-body — neither the Analysis nor the History pane has a footer, because the floating menu covers that space; their actions sit in the pane headers instead. The responsive fallback is a **first-pass requirement, not a fast-follow** — the spec flags cutting it under timeline pressure as a known risk, so build the layout primitives to serve both arrangements from the start.
...
- **The sub-`lg` pane tabs are a menu floating over the bottom of the viewport.** §9 only says "top-level tabs"; putting them at the top made the app's primary phone navigation its least thumb-reachable control. `PaneTabBar` renders them as a `rounded-full` pill of icon+label buttons, `fixed` with a `pointer-events-none` wrapper so only the pill takes taps, and Analyze is both the centre item and `useState`'s initial value — the pane the app opens on is the one under the thumb. The tab is deliberately **not** persisted in `lib/preferences.ts` alongside the other layout prefs, because "always opens on Analyze" is the requirement and persisting it would contradict that on a return visit. Two design-system exceptions live here and nowhere else: `rounded-full` (against §12.4's 4px-everywhere radius, which `globals.css` enforces by pinning `--radius-xs/sm/md/lg` to 4px — `rounded-full` is a separate utility and escapes it) and `shadow-md` (which `globals.css` reserves for toasts and modals).
- **Nothing sits statically under the floating menu, so both pane footers were deleted rather than padded around.** Reserving space works for scrolling content; a pinned action bar is just covered. `AnalyzeButton` moved to the Analysis header and `ClearHistoryButton` (now icon-only) + `ThemeToggle` to the History header — where `ResultsPane` already put `CopyResultsButton` — so all three panes are header-plus-scroll-body and the `min-h-[89px]` duplicated across two files to align the footers' hairlines is gone. The Analysis pane's validation hint moved to the end of its scroll body instead of into a counter: it is pane-level, and compare mode renders two per-textbox counters. Scrolling content reserves `--pane-menu-space` (`globals.css` — the pill's height plus its gutter) as `pb-[var(--pane-menu-space)]` with an `lg:` reset, and `Toast` clears the menu by the same token. Write that padding as `px-6 pt-6 pb-[…]`, never `p-6 pb-[…]` — `cn` has no `tailwind-merge`, so layering over the shorthand relies on Tailwind's sort order to break the tie.
- **The menu hides while a textarea has focus, which is how the mobile keyboard is handled.** A `fixed` element is anchored to the layout viewport, which the soft keyboard does not shrink, and iOS Safari displaces fixed elements while it animates; `visualViewport` geometry is reported inconsistently there and jitters. So `useTextareaFocused` in `PaneTabBar` watches document-level `focusin`/`focusout` and hides the bar (translate + fade + `inert`, so it is not an off-screen tab stop). Two details are load-bearing: `focusout` is read after a `setTimeout(0)`, because during the event `document.activeElement` is browser-dependently `body` or the incoming node and reading it synchronously flashes the bar back for a frame when moving between Text A and Text B — and it is a timer rather than `requestAnimationFrame`, which does not run at all in a backgrounded tab and would leave the bar stuck hidden. The transition names `translate`, not `transform`: Tailwind v4 emits `translate-y-*` on the individual `translate` property, so `transition-[…,transform]` fades but snaps.
