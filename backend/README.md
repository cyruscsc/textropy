...
Most of the 50 features share underlying work. All 35 Tier 1 features want a spaCy parse.
...
where each feature extracts what it needs would parse the same text 35 times and run
...
Worked example — `{"tiers": [1, 3]}` selects 37 computers:

```
all 35 Tier 1 features                        requires  {spacy.doc}
...
One parse and one DistilGPT2 pass, for 37 features. Tier 1 has grown from 5 features to 35
since this document was first written — lemma and density pairs, then the clause, sentence,
punctuation and complexity groups — and the extractor count never moved. That is the property
the design buys.
...
**Tier 1** — 35 features across five modules, every one reading `spacy.doc` and nothing else.
Per-feature definitions, edge cases and the reasoning behind them live in
[`../specifications/specs_features.md`](../specifications/specs_features.md) §2–6, which is the
...
| `tier1/punctuation.py` | 5 | total / internal / terminal punctuation counts and the two ratios |
| `tier1/complexity.py` | 6 | mean dependency distance, tree depth, phrasal elaboration (mean + stdev each) |
